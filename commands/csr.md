---
allowed-tools: Bash(csr:*), AskUserQuestion
argument-hint: [session-name]
description: Delete a Claude Code session (current one, or matched by name) using the csr binary
---
...
1. Run `csr --list` (one Bash call). Each line is `id<TAB>title<TAB>is_current(0/1)<TAB>mtime`.
...
6. On yes, run exactly: `csr --id <id> --yes --no-resume` (one Bash call).
