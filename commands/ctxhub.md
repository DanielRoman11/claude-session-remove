---
allowed-tools: Bash(ctxhub:*), AskUserQuestion
argument-hint: [session-name]
description: Delete a Claude Code session (current one, or matched by name) using the ctxhub binary
---
...
1. Run `ctxhub --list` (one Bash call). Each line is `provider_slug<TAB>id<TAB>provider_name<TAB>title<TAB>is_current(0/1)<TAB>updated_unix`.
...
6. On yes, run exactly: `ctxhub --id <provider_slug>:<id> --yes --no-resume` (one Bash call).
